...
y+=8
vsp=0
mycoll=instance_create(x+32,y+24,moving)
mycoll.mask_index=spr_mask32c32
...
if (inview()) {
    ...
    with (bigmuncher) mycoll.x-=verybignumber
    yg=easyground()-8
    with (bigmuncher) mycoll.x+=verybignumber
    if (y<yg) {
        y-=16
        coll=instance_place(x,y+18,bigmuncher)
